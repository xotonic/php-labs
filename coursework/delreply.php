...
$comments = send_and_check("SELECT id from ccomments where id_reply=$id");
while ($row = mysql_fetch_assoc($comments)) {
    send_and_check("DELETE from crates_comment where id_comment=$row[id]");
}

$replies = send_and_check("SELECT id from creplies where id=$id");
...
*/
send_and_check("DELETE from ccomments where id_reply=$id");
send_and_check("DELETE from creplies where id=$id");
