...
include("util.php");
//enable_debug();
//die("Лаба заблочена на время тестирования 4й лабы");
?>
<html>
<head>
...
<body>
  <?php include '../util.php'; coub("3s3tx"); ?>
<center>
	...
	foreach ($filter as $key => $value) {
		...
		switch ($value) {
			...
			case 'count':
			{
				...
				if ($min) $SQL .= "and models.count > $min ";
				if ($max) $SQL .= "and models.count < $max ";
				break;
			}
			case 'price':
			{
				...
				if ($min) $SQL .= "and models.price > $min ";
				if ($max) $SQL .= "and models.price < $max ";
				break;
			}
			case 'category':
			{
